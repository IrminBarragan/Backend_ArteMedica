...
public interface MovimientoInventarioRepository extends JpaRepository<MovimientoInventario, Long> {
    ...
    @Query(value = "SELECT m FROM MovimientoInventario m JOIN FETCH m.lote JOIN FETCH m.producto "
           ...
           countQuery = "SELECT COUNT(m) FROM MovimientoInventario m "
            + "WHERE m.origenTipo = :origenTipo AND m.origenId = :origenId")
    Page<MovimientoInventario> buscarPorOrigen(@Param("origenTipo") OrigenMovimiento origenTipo,
                                               @Param("origenId") Long origenId, Pageable pageable);

    @Query(value = "SELECT m FROM MovimientoInventario m JOIN FETCH m.lote JOIN FETCH m.producto "
            + "JOIN FETCH m.usuario ORDER BY m.fechaMovimiento DESC",
           countQuery = "SELECT COUNT(m) FROM MovimientoInventario m")
    Page<MovimientoInventario> buscarTodos(Pageable pageable);
}
